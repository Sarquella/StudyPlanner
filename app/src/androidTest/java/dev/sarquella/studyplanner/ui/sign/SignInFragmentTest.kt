...
import dev.sarquella.studyplanner.ui.sign.signIn.SignInFragment
...
class SignInFragmentTest {
    ...
    companion object {
        ...
        private val viewModel: SignInViewModel = mockk(relaxUnitFun = true)
    ...
    @get:Rule
    val fragmentTestRule = FragmentTestRule<SignInFragment>()
    ...
    fun checkStaticUIisCorrect() {
        onView(withId(R.id.tvTitle)).check(matches(withText(R.string.Sign_In)))
        onView(withId(R.id.btSign)).check(matches(withText(R.string.Sign_In)))
        onView(withId(R.id.tvNavToNext)).check(matches(withText(R.string.do_not_have_an_account_yet)))
        onView(withId(R.id.btNavToNext)).check(matches(withText(R.string.Sign_Up)))
        onView(withId(R.id.etPassword)).check(
            matches(
                withInputType(EditorInfo.TYPE_TEXT_VARIATION_PASSWORD or EditorInfo.TYPE_CLASS_TEXT)
            )
        )
    }
    ...
    @Test
    fun whenNavToNextIsTrue_thenNavToSignUpIsPerformed() {
        navToNext.postValue(true)

        makeVerifyWaitForLiveData()

        verify { navController.navigate(R.id.action_nav_to_sign_up) }
    }
}